...
\abstract[Summary]{River-flow forecasting requires predictive distributions that remain informative in both routine and extreme conditions. We develop a Bayesian quantile-based correction-and-synthesis framework built on Dynamic Quantile Linear Models (DQLMs). The framework links United States Geological Survey (USGS) observations, retrospective products, and ensemble forecast products through a shared latent quantile process, learns dynamic discrepancies for each external source, and combines quantile-specific posterior predictions into a single predictive distribution. We also adapt variational Bayes inference to the extended dynamic quantile linear model (exDQLM) using Laplace--Delta approximations for non-conjugate parameters. The methodology is studied using daily flow for the San Lorenzo River together with European Centre for Medium-Range Weather Forecasts/Global Flood Awareness System (ECMWF/GloFAS) and National Oceanic and Atmospheric Administration/National Weather Service (NOAA/NWS) products, with emphasis on medium-range forecasting and uncertainty quantification across multiple quantile levels.}
...
\footnotetext{\textbf{Abbreviations:} AL, asymmetric Laplace; CRPS, continuous ranked probability score; DQLM, dynamic quantile linear model; ECMWF, European Centre for Medium-Range Weather Forecasts; exAL, extended asymmetric Laplace; exDQLM, extended dynamic quantile linear model; FFBS, forward filtering backward sampling; GIG, generalized inverse Gaussian; GloFAS, Global Flood Awareness System; NOAA, National Oceanic and Atmospheric Administration; NWM, National Water Model; NWS, National Weather Service; USGS, United States Geological Survey.}
...
The USGS observations and the retrospective products are treated as source-specific views of the same latent quantile process:
\begin{align}
     \text{USGS observations}:&& y^o_t \mid \boldsymbol{\eta}_t, \sigma^o, \gamma^o &\sim \text{exAL}_{p_0}\!\left(\mathbf{h}_{t,0}' \boldsymbol{\eta}_t, \sigma^o, \gamma^o\right), \label{eq:usgsobs} \\
    \text{Retrospective products}:&& z^j_t \mid \boldsymbol{\eta}_t, \sigma^j, \gamma^j &\sim \text{exAL}_{p_0}\!\left(\mathbf{h}_{t,j}' \boldsymbol{\eta}_t, \sigma^j, \gamma^j\right), \quad 1 \leq j \leq J, \label{eq:retroobs} \\
    \text{All states}:&& \boldsymbol{\eta}_t \mid \boldsymbol{\eta}_{t-1} &\sim \mathcal{N}\!\left(\boldsymbol{D}_t \boldsymbol{\eta}_{t-1}, \boldsymbol{\Lambda}_t\right). \label{eq:stateevolution}
\end{align}
Here,
\[
\mathbf{h}_{t,0} =
...
\quad 1 \le j \le J,
\]
so the USGS mean is the latent quantile \(\mathbf{F}_t' \boldsymbol{\theta}_t + \zeta_t\), whereas the \(j\)th retrospective source adds its discrepancy term \(\mathbf{F}_t' \boldsymbol{\delta}_t^j\). The state evolution matrix and covariance are
\[
\boldsymbol{D}_t = \text{BlockDiag}\!\left(\mathbf{G}_t, \mathbf{G}_t, \ldots, \mathbf{G}_t, \mathbf{G}^{\text{trans}}_t\right),
\]
where \(\text{BlockDiag}(\cdot)\) denotes block-diagonal concatenation, with \(\mathbf{G}_t\) repeated once for the latent trend/seasonal block and once for each discrepancy block, and
\[
...
\]
This formulation makes the main components explicit: a shared latent quantile process, source-specific discrepancy dynamics, and a transfer block driven by external covariates. The earlier observation-only and retrospective-only formulations are nested restrictions of \eqref{eq:usgsobs}--\eqref{eq:stateevolution}: suppressing the discrepancy states recovers the observation-only exDQLM, and restricting attention to \(t \leq T\) gives the retrospective-correction model.

\subsection{Forecast-Window Extension}
\label{subsec:forecastwindow}

After the cutoff, the same state-space framework is propagated forward by replacing the retrospective channels with forecast-product channels. Each ensemble member is treated as one source-specific future trajectory available at the forecast origin.

Let \(K_{(j)}\) denote the horizon length of forecaster \(j\), ordered so that \(K_{(1)} \ge \cdots \ge K_{(J)}\). If forecaster \(j\) provides \(I_j\) ensemble members and \(y_T^{j,i}(k)\) denotes member \(i\) issued at time \(T\) for lead \(k\), then for \(t=T+k\) and each source \(j\) with \(k \le K_{(j)}\),
\begin{align}
    \text{Forecast products}:&& y_T^{j,i}(k) \mid \boldsymbol{\eta}_{T+k}, \sigma^j, \gamma^j &\sim \text{exAL}_{p_0}\!\left(\mathbf{e}_{T+k,j}' \boldsymbol{\eta}_{T+k}, \sigma^j, \gamma^j\right), \label{eq:forecastobs} \\
    \text{Forecast-window states}:&& \boldsymbol{\eta}_{T+k} \mid \boldsymbol{\eta}_{T+k-1} &\sim \mathcal{N}\!\left(\boldsymbol{M}_{T+k} \boldsymbol{\eta}_{T+k-1}, \mathbf{W}_{T+k}\right), \label{eq:forecaststate} \\
    \text{Stochastic covariance}:&& \mathbf{W}_{T+k} &\sim \text{Inverse-Wishart}(\nu_{T+k}, S_{T+k}).
\end{align}
The vector \(\mathbf{e}_{T+k,j}\) is the forecast-window analogue of \(\mathbf{h}_{t,j}\): it extracts the latent trend/seasonal block, the discrepancy block for source \(j\), and, when active, the transfer block. The matrix \(\boldsymbol{M}_{T+k}\) propagates the same components forward into the forecast window. In the active-transfer multivariate exAL specification, the transfer states \((\zeta_t,\boldsymbol{\psi}_t)\) remain active and the forecast covariates available at the cutoff are propagated through the transition block. The reduced no-transfer variant is obtained by suppressing this forecast-window transfer block after the cutoff.

Equations \eqref{eq:usgsobs}--\eqref{eq:forecaststate} define one state-space model with time-dependent observation channels. In implementation, the forecast-window vectors and matrices are reduced to the forecasters whose horizons remain active at a given lead time. This simplifies computation without changing the model.
...
The transfer component uses both local hydrological covariates and a large-scale climate factor. The local covariates are precipitation from the PRISM Climate Group \citep{prism} and soil moisture from the ECMWF ERA5-Land reanalysis \citep{ecmwf}. The ERA5-Land fields are reanalysis-based inputs rather than direct observations, and some variables may include short forecast components.
...
Table~\ref{tab:benchmark_crps_models} compares the raw forecast products with nine Bayesian variants of the common state-space framework. These variants differ only in three respects: the observation likelihood, whether the synthesis uses only the USGS series or the full multivariate data set, and whether the transfer component remains active during the forecast window. We summarize them with the label \(L\)-\(S\)-\(T\), where \(L\in\{\mathrm{N},\mathrm{AL},\mathrm{exAL}\}\) indexes the likelihood, with \(N\) denoting a Gaussian likelihood; \(S\in\{\mathrm{U},\mathrm{M}\}\) indicates univariate or multivariate synthesis; and \(T\in\{\mathrm{T0},\mathrm{T1}\}\) indicates whether the transfer component is inactive or active during the forecast window. The raw baselines are the daily GloFAS and NWM forecast products evaluated without statistical correction.

\begin{table*}[htbp]
\centering
\renewcommand{\arraystretch}{1.08}
\caption{Mean forecast-window CRPS by model family and cutoff across the five rolling-origin evaluation folds. Model labels use \(L\)-\(S\)-\(T\): \(N\), AL, and exAL denote Gaussian, asymmetric Laplace, and extended asymmetric Laplace observation likelihoods; \(U\) and \(M\) denote univariate and multivariate synthesis; and T0 and T1 denote inactive and active forecast-window transfer components. RAW-NWM is the raw National Water Model baseline. Lower values are better; bold indicates the lowest CRPS within each cutoff column.}
...
\end{tabular*}
\begin{tablenotes}
\item \textit{Note:} Posterior means and 95\% credible intervals (CIs) $[{\rm Q2.5}, {\rm Q97.5}]$ for covariate coefficients in the transfer function, at final time $T$, for the 5th, 50th, and 95th quantile models.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
The reusable exDQLM estimation routines are available through the Comprehensive R Archive Network (CRAN) R package \texttt{exdqlm}. The end-to-end forecasting workflow used in this study is available at \url{https://github.com/AntonioAPDL/Project1}. Applying that workflow to a new basin still requires constructing a basin-specific, version-consistent archive of observations, retrospective products, forecast products, and forecast covariates.
...
\end{tabular*}
\begin{tablenotes}
\item \textit{Note:} Entries show the posterior median and the 95\% credible interval (CI) $[{\rm Q2.5}, {\rm Q97.5}]$ for each source and quantile.
\end{tablenotes}
\end{table*}
...
\)
\State \(\mathcal{GIG}\) denotes the generalized inverse Gaussian distribution.
\State For compactness, \(y_t^j\) denotes the active observation or product for source \(j\), and \(\mathbf{h}_{t,j}\) denotes the corresponding active selector vector: the retrospective selector during the estimation period and the forecast-window selector after the cutoff.
\vspace{0.1cm}
...
\State \textbf{Notation:}
\State  $\mathbf{H}_t = [\mathbf{h}_{t,0}, \mathbf{h}_{t,1}, \ldots, \mathbf{h}_{t,J}]$, $\mathbf{Y}_t = [y_t^o, z_t^1, \ldots, z_t^J]'$, and $\mathbf{D}_t, \boldsymbol{\Lambda}_t$ are as defined in Subsection~\ref{subsec:fullstatespace}
...
\)
\State where \(\odot\) is the Hadamard product and \(\mathrm{Diag}(\cdot)\) forms a diagonal matrix
\vspace{0.1cm}
\Statex \rule{\linewidth}{0.4pt}
\vspace{0.1cm}
...
\For{$t = T$ down to $1$}
    ...
    \Else
        ...
        \State $\mathbf{B}_t = \mathbf{C}_t \mathbf{D}_{t+1}' \mathbf{R}_{t+1}^{-1}$
