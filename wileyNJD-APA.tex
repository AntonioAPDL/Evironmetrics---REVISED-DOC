...
\subsection{Shared Latent Quantile Process}
\label{sec:dqlm}

We model the target quantile process with an extended Dynamic Quantile Linear Model (exDQLM), introduced by \citet{barata2021_flex_quantile}. The model combines a latent quantile process for river flow with a transfer-function component for external covariates. For \( 1 \leq t \leq T \), we write
\begin{align}
      \text{USGS observations}:&& y^o_t | \boldsymbol{\theta}_t, \zeta_t, \sigma^o, \gamma^o &\sim \text{exAL}_{p_0} \Big( \mathbf{F}'_t \boldsymbol{\theta}_t + \zeta_t, \sigma^o, \gamma^o \Big) \label{A1} \\
    ...
    \text{Transfer State}:&& \zeta_t | \zeta_{t-1}, \bm{\psi}_t &\sim \mathcal{N} \Big( \lambda \zeta_{t-1} + \mathbf{x}'_t \boldsymbol{\psi}_t, \text{w}^\zeta_t \Big) \label{A3} \\
    \text{Covariate-Effect States}:&& \boldsymbol{\psi}_t | \boldsymbol{\psi}_{t-1} &\sim \mathcal{N} \Big( \boldsymbol{\psi}_{t-1}, \mathbf{W}^\psi_t \Big). \label{A4}
\end{align} 

\noindent Here, $T$ denotes the length of the estimation period, $y^o_t \in {\mathbb{R}}$ denotes the observed river flow at time $t$, $\mathbf{F}_t \in {\mathbb{R}^{p}}$ denotes the observational vector, and $\boldsymbol{\theta}_t \in {\mathbb{R}^{p}}$ denotes the latent state vector. The term $\mathbf{F}'_t \boldsymbol{\theta}_t + \zeta_t$ defines the $p_0$th quantile for the USGS series, with $\boldsymbol{\theta}_t$ governing trend and seasonal dynamics and $\zeta_t$ collecting the accumulated effect of external covariates $\mathbf{x}_t \in {\mathbb{R}^{m}}$. The time-varying covariate coefficients are stored in $\boldsymbol{\psi}_t \in {\mathbb{R}^{m}}$, and the scalar $\lambda$ controls persistence in the transfer component. The source-specific scale and skewness parameters are denoted by $\sigma^o > 0$ and $\gamma^o \in [L,U]$, where the bounds depend on $p_0$ as defined above.

Stacking the latent process into a single state vector gives the compact representation
\begin{align}
    \text{USGS observations}:&& y^o_t | \boldsymbol{\alpha}_t, \sigma^o, \gamma^o & \sim exAL_{p_0} \Big( \Tilde{\mathbf{F}}'_t \boldsymbol{\alpha}_t, \sigma^o, \gamma^o \Big) \label{A5} \\
...
\subsection{Source-Specific Observation Equations}
\label{subsec:retrospective}

Retrospective products are single-valued hydrological reconstructions obtained by repeatedly applying a forecasting and data-assimilation system over a historical period. They are not observations, but they are typically more temporally consistent than real-time forecast products and are therefore useful for bias correction and post-processing. We treat each retrospective series as a source-specific view of the same latent river-flow quantile process.

To incorporate \(J \geq 1\) retrospective products, we augment \eqref{A5}--\eqref{A6} as follows:
\begin{align}
      \text{USGS observations}:&& y^o_t | \boldsymbol{\alpha}_t, \sigma^o, \gamma^o &\sim exAL_{p_0} \Big( \Tilde{\mathbf{F}}'_t \boldsymbol{\alpha}_t, \sigma^o, \gamma^o \Big) \label{B1}  \\
    ...
    \text{Latent process states}:&&\boldsymbol{\alpha}_t | \boldsymbol{\alpha}_{t-1} &\sim \mathcal{N} \Big( \Tilde{\mathbf{G}}_t \boldsymbol{\alpha}_{t-1}, \Tilde{\mathbf{W}}_t \Big) \label{B3}\\
    \text{Discrepancy States}:&& \boldsymbol{\delta}^j_t | \boldsymbol{\delta}^j_{t-1} &\sim \mathcal{N} \Big( \mathbf{G}_t \boldsymbol{\delta}^j_{t-1}, \mathbf{W}^{\delta^j}_t \Big), \quad 1 \leq j \leq J. \label{B4}
\end{align} 
Here, \( z^{j}_t \in {\mathbb{R}} \) denotes the retrospective product from agency \(j\) at time \(t\), and \(\boldsymbol{\delta}^j_t \in {\mathbb{R}}^{p}\) denotes its discrepancy state vector. The shared term \( \Tilde{\mathbf{F}}'_t \boldsymbol{\alpha}_t \) is the \(p_0\)th quantile for the USGS series, whereas \( \Tilde{\mathbf{F}}'_t \boldsymbol{\alpha}_t+\mathbf{F}'_t \boldsymbol{\delta}^j_t \) is the corresponding quantile for retrospective source \(j\). The discrepancy states allow each source to depart from the latent river-flow process in a structured way through time, rather than through a single static bias term. This yields a more interpretable description of regime-dependent source differences and provides the dynamic correction carried into the forecast window. We also assign source-specific scale and skewness parameters \((\sigma^j,\gamma^j)\), since the retrospective systems are generated by different models and post-processing pipelines. Related discrepancy-based forecast-combination strategies appear in \citet{salazar2011comparing,beltran,lemos2009spatio}.

Stacking the latent and discrepancy states gives
\begin{align}
     \text{USGS observations}:&& y^o_t | \boldsymbol{\mu}_t, \sigma^o, \gamma^o &\sim exAL_{p_0} \Big( \textbf{h}_{t,1}' \boldsymbol{\mu}_t, \sigma^o, \gamma^o \Big) \\ \label{B5}
...
\). Here, $\textbf{h}_{t,k}$ denotes the \(k\)th column of \(\mathbf{H}_t\), and \( \boldsymbol{\delta}_t \in {\mathbb{R}}^{pJ} \) denotes the concatenation of the \(J\) discrepancy vectors at time \(t\). We assume conditional independence between \( y^o_t \) and the retrospective products, as well as across retrospective products, given the latent states. A correlated extension is possible through the multivariate exDQLM formulation of \citet{barata2021dqlm}, but we do not pursue that extension here.

\subsection{Forecast-Window Extension}
\label{subsec:ensemble}

Ensemble forecast products are real-time predictive datasets generated by climate and hydrological agencies from the information available at a forecast origin. Each ensemble member represents one plausible future trajectory, so the ensemble provides a source-specific description of forecast uncertainty over the post-cutoff window.

We extend the framework beyond the forecast origin \(T\) by linking the latent process to ensemble forecast products. Let \(K_{(j)}\) denote the horizon length of the products issued by forecaster \(j\), ordered so that \(K_{(1)} \geq \ldots \geq K_{(J)}\). Suppose forecaster \(j\) provides \(I_j \geq 1\) ensemble members, and let \(y^{j,i}_T(k) \in {\mathbb{R}}\) denote member \(i\) from forecaster \(j\), issued at time \(T\) for lead \(k\). For \(t > T\), \(1 \leq j \leq J\), \(1 \leq i \leq I_j\), and \(1 \leq k \leq K_{(j)}\), we use
\begin{align} 
    ...
    \text{Forecast-window States}:&& \boldsymbol{\beta}_{T+k} | \boldsymbol{\beta}_{T+k-1} &\sim \mathcal{N} \Big( \textbf{M}_{T+k} \boldsymbol{\beta}_{T+k-1}, \, \mathbf{W}_{T+k} \Big),  \label{C2} \\
    \text{Stochastic Covariance}:&& \mathbf{W}_{T+k} &\sim \text{InvWishart}(\nu_{T+k}, S_{T+k})
\end{align}
where, for the reduced forecast-window specification used in T0 and for \(T+1+K_{(j+1)} \leq t \leq T+K_{(j)}\),
\(
...
\end{bmatrix}, \quad
\textbf{M}_t = \text{BlockDiag} \Big( \mathbf{G}_t, \stackrel{j+1 \; \text{times}}{\dots}, \mathbf{G}_t  \Big).
\)
Here, \(\textbf{e}_{t,k}\) denotes the \(k\)th column of \(\boldsymbol{E}_t\). The Inverse Wishart parametrization is such that \( \mathbb{E}[\mathbf{W_t}] = \frac{\mathbf{S}_t}{\nu_t - p_t - 1} \), where \(\mathbf{S}_t\) is a positive definite scale matrix, \(\nu_t\) denotes the degrees of freedom, and \(p_t\) is the dimension of \(\boldsymbol{\beta}_t\) at time \(t\). The vector \(\boldsymbol{\delta}^f_t \in \mathbb{R}^{d_t}\) represents the vertical concatenation of the discrepancy state vectors for all forecasters that provide ensemble predictions at time \(t\). Explicitly,
\(
...
Equations \eqref{C1}--\eqref{C2} define the forecast-window state-space specification. In the application, we compare two forecast-window variants. In T0, \(\boldsymbol{\beta}_t\) contains only the trend, seasonal, and discrepancy states. In T1, \(\boldsymbol{\beta}_t\), \(\boldsymbol{E}_t\), and \(\boldsymbol{M}_t\) are augmented with the transfer states \((\zeta_t,\boldsymbol{\psi}_t)\), and forecast covariates available at the cutoff are propagated through the corresponding transition block. The empirical comparison therefore concerns two forecast-window variants of the same framework.
...
Algorithm~\ref{alg1} gives the pseudocode for the estimation-period formulation with retrospective products. The forecast-window extension uses the same conditional updates, with additional bookkeeping to handle the trans-dimensional state vector when forecast horizons differ across agencies. All full conditional posteriors are available in closed form except for the skewness parameters $\gamma^j$, $1 \leq j \leq J$. For these, we use a Metropolis--Hastings random walk on an unconstrained reparameterization. Posterior sampling of the state parameters relies on a modified Forward Filtering Backward Sampling (FFBS) algorithm that leverages their conditional normality (see Algorithm~\ref{alg:Alg2}).
...
Lower quantiles are relevant for drought conditions, whereas upper quantiles are relevant for flood risk. The application therefore combines retrospective products from two climate agencies with local and large-scale covariates to study how external information changes the predicted quantiles over time and during the forecast window.
...
We use retrospective products from two primary sources: the Global Flood Awareness System (GloFAS) operated by ECMWF, and the National Weather Service (NWS) operated by NOAA. Figure~\ref{fig:retrospectives} shows the corresponding retrospective series at the Big Trees station.
...
We choose discount factors componentwise to reflect each component’s time scale and role. Recall that, in the discount-factor formulation \citep{prado2021time}, a smaller value \(d \in (0,1]\) for a given state parameter implies larger state-evolution variance and therefore greater adaptability of that component. We use component discounting. More specifically, we set a static local level with \(d_{\text{trend}}=1\), so the trend behaves effectively as a constant. For seasonality, we favor high persistence across all harmonics---yearly, semestral, and the 80-month cycle---using a common value \(d_{\text{seas}}=0.9997\). This guards against overreacting to short-term fluctuations while allowing slow adjustment of amplitude and phase consistent with multi-year variability and occasional structural changes, such as modifications to local infrastructure. For the discrepancy states, we use \(d_{\text{disc}}=0.999\), providing modest flexibility to adapt to non-systematic differences between the retrospective analyses and the observations across wet and dry regimes. The covariate effects in~\eqref{A4} remain static with discount factors of 1. Finally, the transfer rate is set to \(\lambda=0.8995\), obtained by optimizing one-step-ahead CRPS with the above discount factors held fixed. These choices reflect the intended time scales of each component. In our experiments, discount-factor values above 0.9997 for the seasonal components and above 0.999 for the discrepancies gave qualitatively similar results, while the selected settings provided stable and interpretable dynamics. Smaller discount-factor values made the seasonal components too volatile to interpret and increased the risk of quantile crossing in the discrepancy states.

\subsection{Rolling-Origin Forecast Evaluation Design}

For the forecasting evaluation, we use five rolling-origin cutoff dates that span contrasting hydrological conditions while remaining compatible with the archived forecast products available from each agency. At each cutoff, the model is fit using USGS observations and retrospective products available through that date. Forecasts are then generated over the subsequent forecast window using the latest forecast products issued at or before the cutoff and, for the T1 specification, the forecast covariates available at that origin. Candidate specifications differ in whether the transfer component remains active during that window, but post-cutoff USGS observations are reserved for verification and are not used to fit or update the predictive distributions.

GloFAS issues forecasts daily, whereas NWS issues them hourly. To maintain temporal consistency across sources, we aggregate the NWS products to daily resolution and then retain only the most recent daily forecast available at each cutoff. Figure~\ref{fig:ensembles} illustrates this information structure for the representative cutoff on December 25, 2022. The GloFAS forecast products extend from December 25, 2022 to January 22, 2023, while the NWS forecast products extend from December 25, 2022 to January 5, 2023. The USGS series before the cutoff is used for fitting, and the post-cutoff USGS values are used only to verify the forecast window.
...
Before turning to parameter-level summaries for the reference specification, we compare forecast-window skill across all candidate model families and the raw forecast-product baselines. All nine Bayesian alternatives share the same dynamic backbone; they differ in the observation likelihood, in whether retrospective and forecast products are incorporated, and in whether the transfer component remains active during the forecast window. We use the compact label \(L\)-\(S\)-\(T\): \(L\in\{\mathrm{N},\mathrm{AL},\mathrm{exAL}\}\), where N denotes the Normal Dynamic Linear Model (NDLM), AL denotes the Dynamic Quantile Linear Model with asymmetric Laplace likelihood, and exAL denotes the extended DQLM with extended asymmetric Laplace likelihood; \(S\in\{\mathrm{U},\mathrm{M}\}\), where U is univariate and M is multivariate; and \(T\in\{\mathrm{T0},\mathrm{T1}\}\), where T0 indicates that the transfer function is inactive during the forecast window and T1 indicates that it remains active during the forecast window. We also report the raw GloFAS and NWS forecast-product baselines.

\begin{table*}[htbp]
\centering
\renewcommand{\arraystretch}{1.08}
\caption{Mean forecast-window CRPS by model family and cutoff. Lower values are better; bold indicates the lowest CRPS within each cutoff column.}
...
Table~\ref{tab:benchmark_crps_models} shows a clear ranking pattern. The two multivariate synthesis models with transfer function active during the forecast window (AL-M-T1 and exAL-M-T1) dominate the comparison. Under the comparison reported here, exAL-M-T1 is the best-performing model in all five cutoffs, with AL-M-T1 remaining the closest competitor and particularly close at 01/23/2021 and 05/11/2022. In all five cutoffs, the best Bayesian corrected model outperforms the best raw forecast-product baseline. This comparison highlights the value of combining retrospective analyses and ensemble forecasts within the synthesis framework, and it motivates our detailed focus below on the exAL specification as the most consistently strong model across cutoffs.
...
Overall, the increase of both precipitation and soil--moisture effects toward the upper tail emphasizes their joint role in shaping extremes. This pattern motivates further work on forecast-window transfer specifications that use calibrated probabilistic inputs for these covariates.


\subsection{Posterior Predictive Synthesis}
\label{subsec:synthesis}

To obtain a single predictive distribution from several quantile-specific posterior models, we synthesize their posterior predictive quantiles into one quantile function.
...
Because the quantile models are fit independently, quantile crossing can in principle occur. When needed, we enforce monotonicity by first applying isotonic regression to the fitted quantiles at the target levels \citep{barlow1972} and then applying monotone rearrangement to the interpolated quantile curve \citep{chernozhukov2010}. In our application, no quantile crossing was observed, so the corrected and uncorrected syntheses were nearly identical.
...
\label{fig:synth1}
\end{figure}
Figure~\ref{fig:synth1} shows the synthesized posterior predictive distribution obtained by integrating observed USGS measurements, retrospective analyses, and ensemble forecasts before and after December 25, 2022. Retrospective analyses (from ECMWF and NWS) and ensemble forecasts are displayed as gray curves. Observed USGS measurements available prior to December 25 are shown in green, whereas future measurements are shown in red. Horizontal dashed lines indicate NOAA-defined minor and major flooding thresholds. The 28-day forecast horizon captures a marked increase in projected flows after the cutoff date, with the upper quantiles remaining above the major flood threshold for several days. Some overestimation is still apparent, which is consistent with the difficulty of predicting precipitation at lead times relevant to flood risk management.
...
Figure~\ref{fig:synth2} presents an idealized benchmark synthesized distribution obtained by using only historical USGS measurements, excluding retrospective analyses and ensemble forecasts. This scenario assumes perfect future knowledge of covariate values and no forecast uncertainty. Comparing it with Figure~\ref{fig:synth1} illustrates how forecast and retrospective inputs change the synthesized predictive distribution in the operational setting.

\section{Conclusions}
\label{sec5}

This study presents a Bayesian quantile-based framework for combining direct observations, retrospective analyses, and ensemble forecast products in river-flow forecasting. The framework is designed to quantify and propagate uncertainty across multiple quantile levels while preserving interpretable trend, seasonal, discrepancy, and transfer components.
...
The model’s performance is conditional on the quality and compatibility of the input products, as well as on the structural assumptions used for the discrepancy dynamics and covariate effects. A remaining modeling choice is how strongly to propagate the transfer component into the forecast window, since that step depends on the quality of the forecast covariates and on the compatibility of the external products.

Natural extensions include joint model selection across quantiles, more flexible dynamic structures, and spatial extensions. The same framework can also be used in other settings where observations, retrospective products, and forecast products all contribute distinct information.

This work provides a scalable quantile-based framework for synthesizing historical and forecast information in river-flow prediction.
...
\State \textbf{Notation:}
\State  $\mathbf{H}_t$, $\mathbf{D}_t$ and $\boldsymbol{\Lambda}_t$ are as defined in Subsection~\ref{subsec:retrospective}
...
\begin{biography}{\includegraphics[width=60pt,height=70pt  ]{empty}}{\textbf{Author Name.} This is sample author biography text this is.}
\end{biography}
