...
Our starting point is an extended Dynamic Quantile Linear Model (exDQLM), introduced by \citet{barata2021_flex_quantile}. The model combines a latent quantile process for river flow with a transfer-function component for external covariates. For \( 1 \leq t \leq T \), we write
\begin{align}
...
The formulation in \eqref{C1}--\eqref{C2} describes the common forecast-window backbone. In the application, we compare two forecast-window specifications. In T0, \(\boldsymbol{\beta}_t\) contains only the trend, seasonal, and discrepancy states. In T1, \(\boldsymbol{\beta}_t\), \(\boldsymbol{E}_t\), and \(\boldsymbol{M}_t\) are augmented with the transfer states \((\zeta_t,\boldsymbol{\psi}_t)\), and forecast covariates available at the cutoff are propagated through the corresponding transition block. This keeps the methodology centered on one framework while allowing the empirical comparison to determine whether the transfer component should remain active during forecasting.
...
Before turning to parameter-level summaries for the reference specification, we compare forecast-window skill across all candidate model families and the raw forecast-product baselines. All nine Bayesian alternatives share the same linear dynamic backbone (trend, seasonal, and transfer-function components) and Gaussian state evolution; they differ in the observation likelihood and in whether external retrospective/forecast information from climate institutions is incorporated during the forecast window. We use the compact label \(L\)-\(S\)-\(T\): \(L\in\{\mathrm{N},\mathrm{AL},\mathrm{exAL}\}\), where N denotes the Normal Dynamic Linear Model (NDLM), AL denotes the Dynamic Quantile Linear Model with asymmetric Laplace likelihood, and exAL denotes the extended DQLM with extended asymmetric Laplace likelihood; \(S\in\{\mathrm{U},\mathrm{M}\}\), where U is univariate and M is multivariate; and \(T\in\{\mathrm{T0},\mathrm{T1}\}\), where T0 indicates that the transfer function is inactive during the forecast window and T1 indicates that it remains active during the forecast window. We also report the raw GloFAS and NWS forecast-product baselines.
