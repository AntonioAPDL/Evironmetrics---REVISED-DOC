...
Before turning to parameter-level summaries for the reference specification, we compare forecast-window skill across all candidate model families and the raw physical ensemble baselines. All nine Bayesian alternatives share the same linear dynamic backbone (trend, seasonal, and transfer-function components) and Gaussian state evolution; they differ in the observation likelihood and in whether external retrospective/forecast information from climate institutions is incorporated during the forecast window. We use the compact label \(L\)-\(S\)-\(T\): \(L\in\{\mathrm{N},\mathrm{AL},\mathrm{exAL}\}\), where N denotes the Normal Dynamic Linear Model (NDLM), AL denotes the Dynamic Quantile Linear Model with asymmetric Laplace likelihood, and exAL denotes the extended DQLM with extended asymmetric Laplace likelihood; \(S\in\{\mathrm{U},\mathrm{M}\}\), where U is univariate and M is multivariate; and \(T\in\{\mathrm{T0},\mathrm{T1}\}\), where T0 indicates that the transfer function is inactive during the forecast window and T1 indicates that it remains active during the forecast window. We also report the raw GloFAS and NWS ensemble baselines. For model families with multiple \(\epsilon\)/\(c\)-factor settings, Table~\ref{tab:benchmark_crps_models} reports the best mean CRPS attained within that family at each cutoff, except for AL-M-T1 at 01/23/2021 and 05/11/2022, where we intentionally report the closest AL-M-T1 configurations that remain above exAL-M-T1 (respectively, \texttt{eps90} and \texttt{eps30cf1}) in order to preserve a direct near-boundary comparison between the AL and exAL multivariate transfer-function variants.

\begin{table*}[htbp]
\centering
\renewcommand{\arraystretch}{1.08}
\caption{Mean forecast-window CRPS by model family and cutoff. Lower values are better. For families with multiple \(\epsilon\)/\(c\)-factor settings, the reported entry is the best mean CRPS attained within that family at the cutoff, except for AL-M-T1 at 01/23/2021 and 05/11/2022, where we report the closest AL-M-T1 configurations that remain above exAL-M-T1; bold indicates the lowest CRPS within each cutoff column.}
...
Table~\ref{tab:benchmark_crps_models} shows a clear ranking pattern. The two multivariate synthesis models with transfer function active during the forecast window (AL-M-T1 and exAL-M-T1) dominate the comparison. Under the comparison reported here, exAL-M-T1 is the best-performing model in all five cutoffs, with AL-M-T1 remaining the closest competitor and particularly close at 01/23/2021 and 05/11/2022. This is a conservative comparison, because for those two cutoffs the AL-M-T1 entries are not family-best values but the nearest AL-M-T1 configurations that still lie above exAL-M-T1. In all five cutoffs, the best Bayesian corrected model outperforms the best raw physical ensemble baseline. This comparison highlights the value of combining retrospective analyses and ensemble forecasts within the synthesis framework, and it motivates our detailed focus below on the exAL specification as the most consistently strong model across cutoffs.
